...
adv_train_x = fgsm.generate_np(data_train, eps=epsilon, ord=ord, clip_min=0., clip_max=1.)
adv_test_x = fgsm.generate_np(data_test, eps=epsilon, ord=ord, clip_min=0., clip_max=1.)


#Total datasets consisting of clean and corrupted data
...
#Compile model uisng mean squared error as loss and adam as optimizer
model.compile(loss='mean_squared_error', optimizer='adam')

#Train model using input of clean and corrupted data and fit to clean reconstructions only
model.fit(data_total_train, labels_total_train, validation_data=(data_total_test, labels_total_test), epochs=100, batch_size=256, shuffle=True)

#Save the model
model.save('pp_auto_encoder_fgm_l2_eps1.h5')
