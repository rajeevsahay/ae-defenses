...
def gen_model(data, epochs, num_layers, fileName = None):
	...
	'''classifier = Sequential()
	classifier.add(Dense(100, input_dim=784, activation='relu', kernel_initializer = 'normal', use_bias = True))
	classifier.add(Dense(100, activation='relu', kernel_initializer = 'normal'))
	classifier.add(Dense(10, activation='softmax', kernel_initializer = 'normal'))
	
	optimizer = keras.optimizers.SGD(lr = 0.01, momentum = 0.9)
	classifier.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
	classifier.fit(X_train, Y_train, validation_data=(X_test, Y_test), epochs=100, batch_size=200, shuffle=True) #training
...
def perturb(data, epochs = 10, epsilon = 0.25): #nn architecture, attack
	...
	adv_x = fgsm.generate_np(X_test, eps=epsilon, ord = np.inf, clip_min=0., clip_max=1.)
	return adv_x
	#if visualize == True:
...
def train_test_red_classifier(data, epochs = 10, num_layers = 392, epsilon = 0.25, autoencoder_epochs = 15):
	...
	if epsilon != 0:
		perturbed_x = perturb(data, epochs, epsilon) #perturbs after training classifier for 'epochs' epochs
